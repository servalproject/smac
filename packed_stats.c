			   ...
			   int count,stats_handle *h,int extractAllP,int debug)
{
  ...
  int curve_number=range_decode_equiprobable(c,0x10000);
  ...
  if (debug)
    fprintf(stderr,
	    "storedChildren=%d, nodeAddress= 0x%x\n",
	    storedChildren,nodeAddress);
  ...
  // using curves to represent distribution count is not meaningful.
  n->count=0xffffff-CHARCOUNT;
  ...
  // Extract permutation
  {
    ...
    permutation_decode(c,freqs,CHARCOUNT,h->permutation_curve);
  ...
  // Calculate counts based on curve
  struct probability_vector curve;
  calcCurve(curve_number,NULL,&curve);
  for(i=0;i<CHARCOUNT;i++) freqs[i].b=curve.v[i]; 

  // Recorder frequencies based on permutation of alphabet
  // sorts by .a field first, which is where we have put the 
  // charids, so this sort restores the order of frequencies.
