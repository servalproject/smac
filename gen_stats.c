...
#define MAXDEPTH 16
long long upcount[MAXDEPTH];
long long downcount[MAXDEPTH];
			       ...
			       int pass,int master_curve)
{
  ...
  if (permutation_number==permutation_count) {
	...
	if (freqs[i].b>freqs[i-1].b) upcount[depth]++; else downcount[depth]++;
      }
    }

    permutation_encode(c,freqs,CHARCOUNT,strlen(permutation)/2,master_curve,depth,
		       permutations,permutation_addresses,
		       permutation_count,ftello(out));
